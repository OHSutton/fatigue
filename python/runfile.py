...
import os
import json
from trivia import * #Oliver Sutton's Library
from fatigue_level import * #Oliver Sutton's Library
...
MINIMUM_EAR = 0.3 #Minimum EAR for both the eyes to mark the eyes as open
...
def check_roll_angle(image, lefteyex, lefteyey, righteyex, righteyey):
    ...
    if DETAILED:
        ...
        cv2.putText(image, "roll angle: {}".format(round(rollAngleDegrees,2)), (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
...
def check_yaw_angle(image, uppernosex, uppernosey, lowernosex, lowernosey):
    ...
    if DETAILED:
        ...
        cv2.putText(image, "yaw angle: {}".format(round(yawAngleDegrees,2)), (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
...
def check_pitch_angle(image,
    ...
    Parameters:
            ...
            rightloiwerjawx (int): x value of the lower point of the right jaw (x component of the coordinate for the pixel)
    ...
    if DETAILED:
        ...
        cv2.putText(image, "pitch angle: {}".format(round(pitchAngleDegrees,2)), (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
...
def pulse_motor(pwm):
    set_motor_speed(pwm, 50)
    time.sleep(2)
    set_motor_speed(pwm, 0)
    time.sleep(1)
    set_motor_speed(pwm, 65)
    time.sleep(2)
    set_motor_speed(pwm, 0)
    time.sleep(1)
    set_motor_speed(pwm, 75)
    time.sleep(2)
    set_motor_speed(pwm, 0)
